...
export default function GroupWorkspace({ teacherId }) {
  ...
  const [submissions, setSubmissions] = useState({}) // key `${homeworkId}_${studentId}`
  ...
  const createGroup = async (e) => {
    ...
    const { data, error } = await supabase
      .from('groups')
      .insert({ name: newGroupName.trim(), created_by: teacherId })
      .select()
      .single()
    setCreating(false)
    if (!error) {
  ...
  const loadGroupData = async () => {
    ...
    // Only students whose account is actually approved show up as "members" —
    // otherwise a pending sign-up request looked like an accepted student.
    const { data: members } = await supabase
      .from('group_members')
      .select('student_id, profiles!inner(id, full_name, username, status)')
      .eq('group_id', activeGroup)
      .eq('profiles.status', 'approved')
    setRoster((members || []).map((m) => m.profiles).filter(Boolean))
    ...
    const { data: subs } = await supabase
      ...
      .eq('group_id', activeGroup)
    const map = {}
    ;(subs || []).forEach((s) => (map[`${s.homework_id}_${s.student_id}`] = s))
    setSubmissions(map)
  }
  ...
  const removeStudent = async (student) => {
    if (
      !window.confirm(
        `Remove ${student.full_name} completely? This deletes their account, submissions, and chat history. This can't be undone.`
      )
    )
      return
  ...
  const deleteHomework = async (hw) => {
    if (!window.confirm(`Delete "${hw.title}" completely? This permanently removes the homework, all student submissions, recordings, comments, and uploaded files. This cannot be undone.`)) return
    setBusyAction(`delete-${hw.id}`)
    try {
      const { data: subs } = await supabase.from('submissions').select('screenshot_urls, submission_files, audio_part1_url, audio_part2_url, audio_part3_url').eq('homework_id', hw.id)
      const submissionPaths = []
      for (const sub of subs || []) {
        for (const url of [...(sub.screenshot_urls || []), ...(sub.submission_files || []).map((f) => f.url), sub.audio_part1_url, sub.audio_part2_url, sub.audio_part3_url]) {
          const path = storagePathFromPublicUrl(url, 'submissions')
      ...
      const homeworkPath = storagePathFromPublicUrl(hw.attachment_url, 'homework-files')
      if (homeworkPath) await supabase.storage.from('homework-files').remove([homeworkPath])
      const { error } = await supabase.from('homeworks').delete().eq('id', hw.id)
      if (error) throw error
      setHomeworks((prev) => prev.filter((h) => h.id !== hw.id))
      setSubmissions((prev) => Object.fromEntries(Object.entries(prev).filter(([key]) => !key.startsWith(`${hw.id}_`))))
    } catch (err) {
  ...
  return (
    <div className="flex flex-row gap-4 sm:gap-6">
      <aside className="w-32 sm:w-56 flex-shrink-0 flex flex-col gap-3">
        <div className="text-xs uppercase tracking-wide text-mist font-mono">Your groups</div>
            ...
            renamingId === g.id ? (
              ...
              <div
                ...
                className={`group flex items-center gap-1 rounded-md text-sm transition-colors ${
                  activeGroup === g.id ? 'bg-brass text-onbrass font-medium' : 'bg-panel-2 text-mist hover:text-paper'
                }`}
              >
                <button
      ...
      <section className="flex-1 flex flex-col gap-5 min-w-0">
        {!activeGroup && (
          <p className="text-mist">Create your first group to get started.</p>
        )}

        {activeGroup && (
          <>
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h2 className="font-display text-xl">{activeGroupObj?.name}</h2>
              <PostHomeworkForm
                groupId={activeGroup}
                teacherId={teacherId}
                onPosted={(hw) => {
            ...
            {roster.length === 0 && (
              <p className="text-mist text-sm">
                No approved students in this group yet. Once you approve someone under the
                Approvals tab, they'll show up here.
              </p>
            )}

            {homeworks.length === 0 && roster.length > 0 && (
              <p className="text-mist text-sm">No homework posted yet.</p>
            )}
                ...
                <table className="w-full text-sm border-separate border-spacing-y-2">
                  <thead>
                    <tr className="text-left text-mist text-xs uppercase font-mono">
                      <th className="pr-4 pb-2">Student</th>
                      {homeworks.map((hw) => (
                        <th key={hw.id} className="px-2 pb-2 text-center min-w-[120px]">
                  ...
                  <tbody>
                    {roster.map((student) => (
                      <tr key={student.id} className="bg-panel-2">
                        <td className="px-3 py-2 rounded-l-md font-medium whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <div>
                              {student.full_name}
                              <div className="text-mist text-xs font-mono">@{student.username}</div>
                            </div>
                            <button
                        ...
                        {homeworks.map((hw) => {
                          ...
                          return (
                            <td key={hw.id} className="px-2 py-2 text-center">
                              <button
                              ...
                              >
                                <StampBadge status={getSubmissionStatus(sub, hw.due_date)} />
                              </button>
                            </td>
                          )
                        })}
      ...
      {viewing && <SubmissionPanel {...viewing} onClose={() => setViewing(null)} />}
      {editingHomework && (
        <EditHomeworkModal
