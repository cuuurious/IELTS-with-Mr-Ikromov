    ...
    <div className="min-h-screen bg-ink text-paper flex flex-col">
      <header className="border-b border-line px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <div className="font-display text-lg leading-none">IELTS with Mr Ikromov</div>
            <div className="text-xs text-mist font-mono">
              {profile?.role === 'teacher' ? 'Examiner desk' : 'Candidate portal'}
            </div>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-sm text-mist hidden sm:inline">{profile?.full_name}</span>
          <ThemeToggle />
          <NotificationBell />
